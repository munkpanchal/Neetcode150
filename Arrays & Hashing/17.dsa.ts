...
class MinStack {
    ...
    push(val: number): void {
        this.stack.push(val);
        if (this.auxMinStack.length == 0) {
            this.auxMinStack.push(val);
        } else {
            let currentMin = Math.min(
                val,
                this.auxMinStack[
                    this.auxMinStack.length - 1
                ]
            );
            this.auxMinStack.push(currentMin);
        }
    }

    pop(): void {
        this.stack.pop();
        this.auxMinStack.pop();
    }
